  ...
  return (
    <article className={`plan-card ${featured ? 'featured' : ''}`}>
      ...
      <div className="plan-card-price">
        ...
        {price > 0 && <p className="plan-card-price-period">{period}</p>}
      </div>
      ...
      <ul className="plan-card-features">
        {features.map((feature, index) => (
          <li 
            key={index} 
            className={`plan-card-feature ${!feature.enabled ? 'disabled' : ''}`}
          >
            <span className="plan-card-feature-icon">
              {feature.enabled ? (
                <Check style={{ width: '11px', height: '11px', strokeWidth: '3' }} aria-hidden="true" />
              ) : (
                <X style={{ width: '11px', height: '11px', strokeWidth: '3' }} aria-hidden="true" />
              )}
            </span>
