...
import { TrendingUp, Calendar, MessageCircle, FileText, AlertCircle } from 'lucide-react';
...
export function ChatbotPlanCard({
  ...
  const creditRemainingPercentage = Math.round(((totalCredit - usedCredit) / totalCredit) * 100);
  const fileRemainingPercentage = Math.round(((totalFileChars - usedFileChars) / totalFileChars) * 100);
  ...
  const getStatusBadge = () => {
    ...
    const minPercentage = Math.min(creditRemainingPercentage, fileRemainingPercentage);
    ...
    >
      ...
      >
        <div className="flex items-center justify-between">
          ...
          {/* نمایش وضعیت اعتبار */}
          <div 
            className={`status-badge flex items-center gap-2 px-4 py-2 rounded-xl ${statusBadge.icon ? 'animate-pulse-slow' : ''}`}
            style={{ 
          ...
          >
            {statusBadge.icon && (
              <AlertCircle className="w-5 h-5" style={{ color: statusBadge.color }} />
            )}
            <span 
              className="inline-block w-3 h-3 rounded-full"
              style={{ backgroundColor: statusBadge.color }}
            ></span>
            <span className="text-sm" style={{ color: statusBadge.color, fontWeight: '600' }}>
              {daysLeft} روز مانده
            </span>
          </div>
        </div>
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="text-center">
              <div className="flex items-center justify-center gap-2 mb-2">
                <MessageCircle className="w-5 h-5" style={{ color: creditColor }} />
            ...
            <div className="text-center">
              ...
              <div className="text-grey-900">
                <span className="text-xl" style={{ color: fileColor }}>
                  {((totalFileChars - usedFileChars) / 1000).toLocaleString('fa-IR')}K
                </span>
                <span className="text-grey-500 mr-1">
