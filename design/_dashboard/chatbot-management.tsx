...
import { Edit3, Trash2, Settings, Eye, Copy, MoreVertical, Palette, MessageSquare, Upload, Save, X, Plus } from 'lucide-react';
...
type PageType = 'landing' | 'signup' | 'dashboard' | 'consultation' | 'chatbot-management';
...
export function ChatbotManagement({ onNavigate }: ChatbotManagementProps) {
  const [chatbots, setChatbots] = useState<Chatbot[]>([
    {
      ...
      fallbackMessage: 'متأسفانه نمی‌تونم پاسخ این سؤال رو بدم. لطفاً با پشتیبانی تماس بگیرید.',
      branding: {
    ...
    },
    {
      ...
      language: 'fa',
      tone: 'expert',
      welcomeMessage: 'با توجه به سؤال شما، پاسخ دقیق این موضوع عبارت است از...',
  ...
  const [activeTab, setActiveTab] = useState<'basic' | 'knowledge' | 'design' | 'messages'>('basic');
  const [newKnowledgeItem, setNewKnowledgeItem] = useState<Partial<KnowledgeSource>>({
    type: 'faq',
    title: '',
    content: ''
  });
  ...
  const addKnowledgeItem = () => {
    if (newKnowledgeItem.title && newKnowledgeItem.content && editForm.knowledgeBase) {
      const updatedKnowledge = [...editForm.knowledgeBase, {
        ...newKnowledgeItem,
          ...
          <div className="max-w-7xl mx-auto pb-8">
            ...
            <header className="mb-8">
              <div className="text-right">
                ...
                <p className="text-grey-600">مدیریت و ویرایش تمام چت بات‌های آیوا</p>
              </div>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
              <div className="bg-white rounded-2xl p-6 border border-grey-100 shadow-card">
                ...
                <h3 className="text-2xl font-bold text-grey-900 mb-1">{chatbots.length}</h3>
              ...
              <div className="bg-white rounded-2xl p-6 border border-grey-100 shadow-card">
                ...
                <h3 className="text-2xl font-bold text-grey-900 mb-1">
                  {chatbots.reduce((sum, bot) => sum + bot.conversationsToday, 0)}
                </h3>
              ...
              <div className="bg-white rounded-2xl p-6 border border-grey-100 shadow-card">
                ...
                <h3 className="text-2xl font-bold text-grey-900 mb-1">
                  {Math.round(chatbots.reduce((sum, bot) => sum + bot.responseRate, 0) / chatbots.length)}%
                </h3>
            ...
            <div className="bg-white rounded-3xl border border-grey-100 shadow-card overflow-hidden">
              <div className="p-6 border-b border-grey-100">
                <h2 className="font-bold text-grey-900 text-xl">لیست چت بات‌ها</h2>
              </div>

              <div className="divide-y divide-grey-100">
                {chatbots.map((chatbot) => (
                  <div key={chatbot.id} className="p-6 hover:bg-grey-50/50 transition-colors">
                    {editMode === chatbot.id ? (
                      // Advanced Edit Mode
                      <div className="space-y-6">
                        ...
                        <div className="flex items-center justify-between pb-4 border-b border-grey-200">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h3 className="font-bold text-grey-900 text-lg">ویرایش چت بات: {chatbot.name}</h3>
                              <p className="text-grey-600 text-sm">تنظیمات کامل چت بات خود را مدیریت کنید</p>
                            </div>
                          </div>
                          <button
                        ...
                        <div className="flex space-x-1 bg-grey-100 p-1 rounded-xl">
                          {[
                            ...
                            { id: 'knowledge', label: 'پایگاه دانش', icon: MessageSquare },
                        ...
                        <div className="min-h-[400px]">
                          {activeTab === 'basic' && (
                            <div className="space-y-6">
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                  <label className="block text-sm font-medium text-grey-700 mb-2">
                                    نام چت بات <span className="text-brand-primary">*</span>
                                  </label>
                                  <Input
                                    value={editForm.name || ''}
                                ...
                                <div>
                                  <label className="block text-sm font-medium text-grey-700 mb-2">آدرس وب‌سایت</label>
                                  <Input
                                    value={editForm.url || ''}
                              ...
                              <div>
                                <label className="block text-sm font-medium text-grey-700 mb-2">توضیحات</label>
                                <textarea
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                  <label className="block text-sm font-medium text-grey-700 mb-2">زبان</label>
                                  <Select
                                ...
                                <div>
                                  <label className="block text-sm font-medium text-grey-700 mb-2">لحن گفتگو</label>
                                  <Select
                                    value={editForm.tone || ''}
                                    onValueChange={(value) => setEditForm(prev => ({ ...prev, tone: value }))}
                          ...
                          {activeTab === 'knowledge' && (
                            <div className="space-y-6">
                              <div className="flex items-center justify-between">
                                <h4 className="font-medium text-grey-900">پایگاه دانش چت بات</h4>
                              ...
                              <Card className="p-4 bg-grey-50">
                                <h5 className="font-medium text-grey-900 mb-4">افزودن دانش جدید</h5>
                                <div className="space-y-4">
                                  <div className="grid grid-cols-2 gap-4">
                                    <Select
                                      value={newKnowledgeItem.type || 'faq'}
                                      onValueChange={(value) => setNewKnowledgeItem(prev => ({ ...prev, type: value as any }))}
                                    >
                                    ...
                                    <Input
                                      ...
                                      onChange={(e) => setNewKnowledgeItem(prev => ({ ...prev, title: e.target.value }))}
                                    />
                                  </div>
                                  <textarea
                                    placeholder="محتوا..."
                                  ...
                                  <Button 
                                    ...
                                    disabled={!newKnowledgeItem.title || !newKnowledgeItem.content}
                                    size="small"
                                  >
                                    ...
                                    <div className="flex items-start justify-between">
                                      <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-2">
                                          ...
                                          <h6 className="font-medium text-grey-900">{item.title}</h6>
                                        </div>
                                        <p className="text-grey-600 text-sm line-clamp-2">{item.content}</p>
                                      </div>
                                      <button
                          ...
                          {activeTab === 'design' && (
                            <div className="grid lg:grid-cols-2 gap-6">
                              <div className="space-y-6">
                                {/* Color Selection */}
                                <Card className="p-4">
                                  <h5 className="font-medium text-grey-900 mb-4">رنگ اصلی</h5>
                                ...
                                {/* Position & Size */}
                                <Card className="p-4">
                                  <h5 className="font-medium text-grey-900 mb-4">موقعیت و اندازه</h5>
                                  <div className="space-y-4">
                                    <div>
                                      <label className="block text-sm text-grey-700 mb-2">موقعیت</label>
                                        ...
                                        {onboardingData.positionOptions.map((position) => (
                                          <button
                                            ...
                                            onClick={() => setEditForm(prev => ({ 
                                              ...prev, 
                                              branding: { ...prev.branding!, position: position.id as any }
                                            }))}
                                            className={`p-3 rounded-lg border-2 transition-all text-sm ${
                                              editForm.branding?.position === position.id
                                    ...
                                    <div>
                                      <label className="block text-sm text-grey-700 mb-2">اندازه</label>
                                          ...
                                          >
                                            ...
                                            <div 
                                              ...
                                              style={{ 
                                                ...
                                                width: `${getSizeInPx(sizeOption.id) / 3}px`,
                                                height: `${getSizeInPx(sizeOption.id) / 3}px`
                                              }}
                                            >
                                              💬
                                            </div>
                              ...
                              {/* Live Preview */}
                              <Card className="p-4">
                                <h5 className="font-medium text-grey-900 mb-4">پیش‌نمایش زنده</h5>
                                <div className="relative bg-white border rounded-lg p-4 min-h-[300px]">
                                  ...
                                  <div 
                                    className={`absolute rounded-full shadow-lg flex items-center justify-center text-white font-medium ${
                                      editForm.branding?.position === 'bottom-right' ? 'bottom-4 left-4' : 'bottom-4 right-4'
                                    }`}
                                    style={{ 
                                      backgroundColor: editForm.color,
                                      width: `${getSizeInPx(editForm.branding?.size || 'medium')}px`,
                                      height: `${getSizeInPx(editForm.branding?.size || 'medium')}px`,
                                      fontSize: `${getSizeInPx(editForm.branding?.size || 'medium') / 3}px`
                                    }}
                                  >
                                    💬
                                  </div>
                          ...
                          {activeTab === 'messages' && (
                            <div className="space-y-6">
                              <div>
                                <label className="block text-sm font-medium text-grey-700 mb-2">
                                  پیام خوش‌آمدگویی <span className="text-brand-primary">*</span>
                                </label>
                                <textarea
                                ...
                                />
                                <p className="text-grey-500 mt-1 text-sm">اولین پیامی که کاربران می‌بینند</p>
                              </div>

                              <div>
                                <label className="block text-sm font-medium text-grey-700 mb-2">پیام عدم درک سؤال</label>
                                <textarea
                                  value={editForm.fallbackMessage || ''}
                                  onChange={(e) => setEditForm(prev => ({ ...prev, fallbackMessage: e.target.value }))}
                                  rows={3}
                                  className="w-full px-4 py-3 border border-grey-200 rounded-xl focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary transition-all resize-none"
                                  placeholder="متأسفانه نمی‌تونم پاسخ این سؤال رو بدم."
                                />
                                <p className="text-grey-500 mt-1 text-sm">زمانی که دستیار نمی‌تواند پاسخ دهد</p>
                              </div>

                              {/* Message Preview */}
                              <Card className="p-4 bg-grey-50">
                                <h5 className="font-medium text-grey-900 mb-4">پیش‌نمایش پیام‌ها</h5>
                                <div className="space-y-3">
                                  <div className="flex justify-start">
                                    <div 
                                      className="max-w-xs p-3 rounded-lg text-white text-sm"
                                      style={{ backgroundColor: editForm.color }}
                                    >
                                      {editForm.welcomeMessage || 'پیام خوش‌آمدگویی'}
                                    </div>
                                  </div>
                                  ...
                                  <div className="flex justify-start">
                                    <div 
                                      ...
                                      style={{ backgroundColor: editForm.color }}
                                    >
                                      {editForm.fallbackMessage || 'پیام عدم درک'}
                                    </div>
                                  </div>
                                </div>
                              </Card>
                        ...
                        <div className="flex gap-3 pt-4 border-t border-grey-200">
                          <Button onClick={handleSaveEdit} className="bg-brand-primary text-white">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <div className="flex items-center gap-3 mb-2">
                              <h3 className="font-bold text-grey-900 text-lg">{chatbot.name}</h3>
                              <span 
                                className="px-3 py-1 rounded-full text-xs font-medium"
                                style={{ 
                                  backgroundColor: `${getStatusColor(chatbot.status)}15`,
                            ...
                            <p className="text-grey-600 text-sm mb-1">{chatbot.url}</p>
                            {chatbot.description && (
                              <p className="text-grey-500 text-sm">{chatbot.description}</p>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-6">
                          ...
                          <div className="text-center">
                            <p className="text-lg font-bold text-grey-900">{chatbot.conversationsToday}</p>
                            <p className="text-xs text-grey-500">گفتگو امروز</p>
                          </div>
                          <div className="text-center">
                            <p className="text-lg font-bold text-grey-900">{chatbot.responseRate}%</p>
                            <p className="text-xs text-grey-500">نرخ پاسخ</p>
                          </div>
                          <div className="text-center">
                            <p className="text-sm font-medium text-grey-700">{chatbot.lastUpdated}</p>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              title={chatbot.status === 'active' ? 'غیرفعال کردن' : 'فعال کردن'}
                            >
