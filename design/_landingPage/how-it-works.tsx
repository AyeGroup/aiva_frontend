...
export function HowItWorks() {
  ...
  return (
    <section className="py-20 md:py-32 relative" aria-labelledby="how-it-works-title">
      {/* Sharp Background */}
      <div className="absolute inset-0" style={{ background: 'var(--sharp-primary)', opacity: 0.05 }}>
        <div className="absolute top-1/4 right-1/4 w-64 h-64 opacity-30 rounded-full" style={{
      ...
      <div className="container mx-auto px-6 relative z-10">
        <header className="text-center mb-20">
          ...
          <div className="inline-flex items-center gap-2 bg-white/90 backdrop-blur-sm border border-brand-primary/15 px-4 py-2 rounded-full shadow-sm mb-8">
            ...
            <span className="text-sm text-brand-primary font-medium">فرایند ساده</span>
          </div>
        ...
        <div className="relative">
          ...
          <div className="hidden lg:block absolute top-20 left-0 w-full h-px" style={{
                ...
                <div className="relative bg-white/95 backdrop-blur-sm border border-white/60 rounded-xl p-8 shadow-sm hover:shadow-md transform hover:-translate-y-1 transition-all duration-300">
                  ...
                  <div className="flex items-center justify-center mb-6 relative">
                    <div className={`w-16 h-16 rounded-xl flex items-center justify-center relative`} style={{
                      background: index % 2 === 0 ? 'var(--sharp-emerald)' : 'var(--sharp-coral)',
                      opacity: 0.15
                    }}>
                      ...
                      {/* Step Number */}
                      <div 
                        className="absolute -top-2 -right-2 w-6 h-6 bg-brand-primary text-white rounded-full flex items-center justify-center text-xs font-bold shadow-sm"
                      >
                        {step.id}
                      </div>
                    </div>
                  ...
                  {/* Content */}
                  <h3 id={`step-${step.id}-title`} className="text-grey-900 mb-4 font-medium">
                    {step.title}
                  </h3>
