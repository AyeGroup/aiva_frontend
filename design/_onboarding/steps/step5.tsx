...
import { Copy, Download, Globe, Code, CheckCircle2, ExternalLink, Settings, BarChart3 } from 'lucide-react';
...
export function WizardStep5({ botConfig, onNavigate }: WizardStep5Props) {
  ...
  const [deploymentMethod, setDeploymentMethod] = useState<'script' | 'wordpress' | 'shopify'>('script');
    ...
    <div className="step-content space-y-8" dir="rtl">
      ...
      <div className="text-right mb-8">
        <div className="flex items-center gap-4 mb-6">
          <div className="w-16 h-16 bg-gradient-to-tr from-success to-brand-secondary rounded-3xl flex items-center justify-center shadow-lg animate-pulse">
            <CheckCircle2 className="w-10 h-10 text-white" />
          </div>
          <div>
            <h2 className="text-grey-900 mb-2 gradient-text">
        ...
        <div className="bg-gradient-to-l from-success/5 to-brand-secondary/5 rounded-2xl p-4 border border-success/20">
          <div className="flex items-center gap-3">
            <svg className="w-6 h-6 text-success" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
            </svg>
            <span className="text-success font-medium">همه چیز آماده! حالا وقت نصب است</span>
          </div>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                ...
                <span className="text-grey-900 font-medium">{botConfig.name}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-grey-900 font-medium">{botConfig.knowledge.length} مورد</span>
              </div>
              <div className="flex justify-between">
                <span className="text-grey-600">موقعیت نمایش:</span>
                <span className="text-grey-900 font-medium">
                  {botConfig.branding.position === 'bottom-right' ? 'پایین راست' : 'پایین چپ'}
                </span>
              </div>
            </div>
          </div>
      ...
      <Card className="p-6">
        ...
        <div className="grid md:grid-cols-3 gap-3 mb-6">
          {[
            { id: 'script', title: 'HTML/JavaScript', desc: 'برای اکثر سایت‌ها', icon: Globe },
            { id: 'wordpress', title: 'WordPress', desc: 'پلاگین اختصاصی', icon: Globe },
            { id: 'shopify', title: 'Shopify', desc: 'فروشگاه آنلاین', icon: Globe }
          ].map((method) => {
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div>
            <h4 className="text-grey-900 mb-3">راهنمای نصب - {instructions.title}</h4>
            ...
            <div className="mt-6 p-4 bg-brand-tertiary/10 border border-brand-tertiary/20 rounded-lg">
              <p className="text-brand-tertiary text-body-small">
                💡 نکته: پس از نصب، ممکن است تا ۱۰ دقیقه طول بکشد تا دستیار در سایت شما فعال شود.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card className="p-6">
          ...
          <p className="text-grey-600 mb-4 text-body-small">
            با رفتن به داشبورد می‌توانید عملکرد دستیار را مشاهده و تنظیمات را بهبود دهید.
          </p>
          <Button 
            variant="primary" 
        ...
        <Card className="p-6">
          ...
          <p className="text-grey-600 mb-4 text-body-small">
            بعداً می‌توانید تنظیمات دستیار را تغییر دهید، دانش جدید اضافه کنید و عملکرد را بهبود دهید.
          </p>
      ...
      <div className="text-center py-8">
        <div className="inline-flex items-center gap-3 px-6 py-3 bg-success/10 border border-success/20 rounded-full">
          ...
          <span className="text-success font-medium">آیوا با موفقیت راه‌اندازی شد!</span>
        </div>
      </div>
    </div>
  );
