...
export function OnboardingWizard({ onNavigate }: OnboardingWizardProps) {
  ...
  const renderCurrentStep = () => {
    switch (currentStep) {
      case 1:
        return <WizardStep1 botConfig={botConfig} updateConfig={updateBotConfig} />;
      case 2:
        return <WizardStep2 botConfig={botConfig} updateConfig={updateBotConfig} />;
      case 3:
        return <WizardStep3 botConfig={botConfig} updateConfig={updateBotConfig} />;
      case 4:
        return <WizardStep4 botConfig={botConfig} updateConfig={updateBotConfig} />;
      case 5:
    ...
    <main className="onboarding-wizard min-h-screen bg-bg-app" dir="rtl">
      <div className="container mx-auto px-6 py-12">
        ...
        <div className="text-center mb-16">
          ...
          <h1 className="text-grey-900 mb-4">
            {title}
          </h1>
          <p className="text-grey-700 max-w-lg mx-auto">
            {subtitle}
          </p>
        </div>
                ...
                return (
                  <div key={step.id} className="flex flex-col items-center">
                    <button
                      ...
                      className={`
                        flex items-center justify-center w-10 h-10 rounded-full transition-all duration-200 text-sm font-medium border-2 shadow-sm
                        ${isActive 
                      ...
                      `}
                      style={isActive ? { background: 'var(--sharp-primary)' } : {}}
                    ...
                    <p className={`mt-3 text-sm text-center max-w-20 font-medium ${
                      isActive 
                        ? 'text-brand-primary' 
                        : isCompleted 
                        ? 'text-success'
